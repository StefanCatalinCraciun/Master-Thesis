...
def DiffDxminus_numba(nx, ny, l, w, A, dA, dx):
        """
        ...
        for i in prange(0, l):
            for j in prange(0, ny):
                ...
                sum = 0.0
                for k in prange(1, i + 1):
                    sum -= w[k - 1] * A[i - k, j]
                for k in prange(1, l + 1):
                    sum += w[k - 1] * A[i + (k - 1), j]
                dA[i, j] = sum / dx
        
        # Outside border area (l <= i < nx - l)
        for i in prange(l, nx - l):
            for j in prange(0, ny):
        ...
        for i in prange(nx - l, nx):
            for j in prange(0, ny):
                # Calculate the weighted sum for right border elements
                sum = 0.0
                for k in prange(1, l + 1):
                    sum -= w[k - 1] * A[i - k, j]
                for k in prange(1, nx - i + 1):
                    sum += w[k - 1] * A[i + (k - 1), j]
                dA[i, j] = sum / dx

@jit(nopython=True, fastmath=True)
def DiffDxplus_numba(nx, ny, l, w, A, dA, dx):
    """
    ...
    for i in prange(nx - l, nx):
        for j in prange(0, ny):
            ...
            for k in prange(1, l + 1):
                sum -= w[k - 1] * A[i - (k - 1), j]
            for k in prange(1, nx - i):
                sum += w[k - 1] * A[i + k, j]
            
            dA[i, j] = sum / dx
            
@jit(nopython=True, fastmath=True)
def DiffDyminus_numba(nx, ny, l, w, A, dA, dx):
    """
    ...
    # Bottom border
    for i in prange(0, nx):
        for j in prange(ny - l, ny):
            sum = 0.0
            for k in prange(1, l + 1):
                sum -= w[k - 1] * A[i, j - k]
            for k in prange(1, ny - j + 1):
                sum += w[k - 1] * A[i, j + (k - 1)]
            dA[i, j] = sum / dx

@jit(nopython=True, fastmath=True)
def DiffDyplus_numba(nx, ny, l, w, A, dA, dx):
    """
...
class Differentiator:
    ...
    def DiffDxminus(Diff, A, dA, dx):
        ...
        DiffDxminus_numba(nx, ny, l, w, A, dA, dx)
    ...
    def DiffDxplus(Diff, A, dA, dx):
        ...
        DiffDxplus_numba(nx, ny, l, w, A, dA, dx)
    ...
    def DiffDyminus(Diff, A, dA, dx):
        ...
        DiffDyminus_numba(nx, ny, l, w, A, dA, dx)
    ...
    def DiffDyplus(Diff, A, dA, dx):
        ...
        DiffDyplus_numba(nx, ny, l, w, A, dA, dx)
