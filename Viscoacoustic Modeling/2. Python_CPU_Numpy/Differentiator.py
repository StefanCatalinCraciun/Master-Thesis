...
class Differentiator:
    ...
    def DiffDxminus(Diff, A, dA, dx):
        ...
        i_indices = np.arange(l)
        k_indices_left = np.arange(1, l+1)
        k_indices_right = np.arange(1, l+1)

        # Calculate weighted sum for left border elements
        sums = np.zeros((l, ny))
        for k in k_indices_left:
            sums -= w[k-1] * A[i_indices-k, :]
        for k in k_indices_right:
            sums += w[k-1] * A[i_indices+(k-1), :]

        # Divide by dx to calculate dA
        dA[i_indices, :] = sums / dx
        ...
        '''
        # Create arrays for indexing
        indices = np.arange(l, nx-l)
        k_indices = np.arange(1, l+1)
        # Calculate weighted sum for elements outside the border area
        sums = np.zeros((len(indices), ny))
        for k in k_indices:
            sums += w[k-1] * (-A[indices-k, :] + A[indices+(k-1), :])
        #Divide by dx to calculate dA
        dA[indices, :] = sums / dx
        ...
        i_indices = np.arange(nx - l, nx)
        k_indices_left = np.arange(1, l + 1)

        # Calculate weighted sum for right border elements
        sums = np.zeros((l, ny))
        for k in k_indices_left:
            sums -= w[k - 1] * A[i_indices - k, :]
        for i, idx in enumerate(i_indices):
            k_indices_right = np.arange(1, nx - idx)
            for k in k_indices_right:
                sums[i, :] += w[k - 1] * A[idx + (k - 1), :]
    ...
    def DiffDxplus(Diff, A, dA, dx):
        ...
        # Create arrays for indexing
        i_indices = np.arange(l)
        k_indices_left = np.arange(1, i_indices[-1]+2)
        k_indices_right = np.arange(1, l+1)
        ...
        indices = np.arange(l, nx-l)
        k_indices = np.arange(1, l+1)
        # Calculate weighted sum for elements between left and right border
        sums = np.zeros((len(indices), ny))
        for k in k_indices:
            sums += w[k-1] * (-A[indices-(k-1), :] + A[indices+k, :])
        # Divide by dx to calculate dA
        dA[indices, :] = sums / dx
    ...
    def DiffDyminus(Diff, A, dA, dx):
        ...
        # Calculate weighted sum for top border elements
        sums = np.zeros((nx, l))
        for k in k_indices_left:
            sums -= w[k-1] * A[:, j_indices-k]
        for k in k_indices_right:
            sums += w[k-1] * A[:, j_indices+(k-1)]

        # Divide by dx to calculate dA
        dA[:, j_indices] = sums / dx
        ...
        '''
        # Create arrays for indexing
        indices = np.arange(l, ny-l)
        k_indices = np.arange(1, l+1)

        # Calculate weighted sum for elements outside the border area
        sums = np.zeros((nx, len(indices)))
        for k in k_indices:
            sums += w[k-1] * (-A[:, indices-k] + A[:, indices+(k-1)])
        ...
        j_indices = np.arange(ny - l, ny)
        k_indices_top = np.arange(1, l + 1)

        # Calculate weighted sum for bottom border elements
        sums = np.zeros((nx, l))
        for k in k_indices_top:
            sums -= w[k - 1] * A[:, j_indices - k]
        for j, idx in enumerate(j_indices):
            k_indices_bottom = np.arange(1, ny - idx)
            for k in k_indices_bottom:
                sums[:, j] += w[k - 1] * A[:, idx + (k - 1)]
    ...
    def DiffDyplus(Diff, A, dA, dx):
        ...
        # Create arrays for indexing
        j_indices = np.arange(l)
        k_indices_left = np.arange(1, j_indices[-1]+2)
        k_indices_right = np.arange(1, l+1)
