...
class Src:
    def __init__(self, source, sx, sy):
        ...
        '''
        self.Src = np.asarray(source)     # Source values (Wavelet shape)
        self.Sx = np.asarray(sx)          # X coordinates of the source
        self.Sy = np.asarray(sy)          # Y coordinates of the source
        self.Ns = len(sx)               # Number of sources (1 use just 1 source)
