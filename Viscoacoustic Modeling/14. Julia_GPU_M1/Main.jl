...
function main()
    ...
    #------------------------------
    nx      = 101#500      # No of grdipoints in x-direction
    ny      = 101#174      # No of gridpoints in y-direction
    dx      = 10           # Grid interval
    ...
    gy = convert(Int, round(ny/2)) + 20#+ 300        # y-position of receiver
    ...
    wavelet = zeros(Float32, nt) # Initialize source pulse
    ...
    Lenght = Float32(Length)
    dt = Float32(dt)

    # Create a Ricker wavelet
    ricker_wavelet = SrcRicker(t0, f0, Length, dt)
    ricker_wavelet = Float32.(ricker_wavelet)
    sx = Float32.(sx)
    sy = Float32.(sy)
    ...
    rec = Rec(Int64.(rx), Int64.(ry), nt, resamp, sresamp, nx, ny, Float32(dx), Nb, dt, gx, gy)
