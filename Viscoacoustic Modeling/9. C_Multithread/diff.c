...
void DiffDxminus(Diff* diff, double **A, double **dA, double dx, int nx, int ny){
    ...
    //
    l = diff->l;
    #pragma omp parallel for default(none) private(i, j, k, sum) shared(A,dA,diff,nx,ny,dx,l)
    for (i=0; i<l; i++){
        for (j=0; j<ny; j++){
            sum=0.0;
            for(k=1; k<i+1; k++){sum = -diff->w[k-1]*A[i-k][j] + sum;}
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*A[i+(k-1)][j] +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=l; i<nx-l; i++){
        for (j=0; j<ny; j++){
            sum=0.0;
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*(-A[i-k][j]+A[i+(k-1)][j]) +sum; }
            dA[i][j] = sum/dx;
        }  
    }
    ...
    for (i=nx-l; i<nx; i++){
        for (j=0; j<ny; j++){
            sum = 0.0;
            for(k=1; k<l+1; k++){sum = -diff->w[k-1]*A[i-k][j] + sum;}
            for(k=1; k<(nx-i+1); k++){sum = diff->w[k-1]*A[i+(k-1)][j] + sum;}
            dA[i][j] = sum/dx;
        }
    }
}
...
void DiffDyminus(Diff* diff, double **A, double **dA, double dx, int nx, int ny){
    ...
    for (i=0; i<nx; i++){
        for (j=0; j<l; j++){
            sum=0.0;
            for(k=1; k<j+1; k++){sum = -diff->w[k-1]*A[i][j-k] + sum;}
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*A[i][j+(k-1)] +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=0; i<nx; i++){
        for (j=l; j<ny-l; j++){
            sum=0.0;
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*(-A[i][j-k]+A[i][j+(k-1)]) +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=0; i<nx; i++){
        for (j=ny-l; j<ny; j++){
            sum = 0.0;
            for(k=1; k<l+1; k++){sum = -diff->w[k-1]*A[i][j-k] + sum;}
            for(k=1; k<(ny-j+1); k++){sum = diff->w[k-1]*A[i][j+(k-1)] + sum;}
            dA[i][j] = sum/dx;
        }
    }
}
...
void DiffDxplus(Diff* diff, double **A, double **dA, double dx, int nx, int ny){
    ...
    for (i=0; i<l; i++){
        for (j=0; j<ny; j++){
            sum=0.0;
            for(k=1; k<i+2; k++){sum = -diff->w[k-1]*A[i-(k-1)][j] + sum;}
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*A[i+k][j] +sum;}
            dA[i][j] = sum/dx;
        } 
    } 
    ...
    for (i=l; i<nx-l; i++){
        for (j=0; j<ny; j++){
            sum=0.0;
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*(-A[i-(k-1)][j]+A[i+k][j]) +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=nx-l; i<nx; i++){
        for (j=0; j<ny; j++){
            sum = 0.0;
            for(k=1; k<l+1; k++){sum = -diff->w[k-1]*A[i-(k-1)][j] + sum;}
            for(k=1; k<nx-i; k++){sum = diff->w[k-1]*A[i+k][j] + sum;}
            dA[i][j] = sum/dx;
        }
    } 
}
...
void DiffDyplus(Diff* diff, double **A, double **dA, double dx, int nx, int ny){
    ...
    for (i=0; i<nx; i++){
        for (j=0; j<l; j++){
            sum=0.0;
            for(k=1; k<j+2; k++){sum = -diff->w[k-1]*A[i][j-(k-1)] + sum;}
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*A[i][j+k] +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=0; i<nx; i++){
        for (j=l; j<ny-l; j++){
            sum=0.0;
            for(k=1; k<l+1; k++){sum = diff->w[k-1]*(-A[i][j-(k-1)]+A[i][j+k]) +sum;}
            dA[i][j] = sum/dx;
        }
    } 
    ...
    for (i=0; i<nx; i++){
        for (j=ny-l; j<ny; j++){
            sum = 0.0;
            for(k=1; k<l+1; k++){sum = -diff->w[k-1]*A[i][j-(k-1)] + sum;}
            for(k=1; k<ny-j; k++){sum = diff->w[k-1]*A[i][j+k] + sum;}
            dA[i][j] = sum/dx;
        }
    }
